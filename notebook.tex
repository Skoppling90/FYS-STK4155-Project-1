...
\makeatother
    ...
    \section{Project number 1}\label{project-number-1}

    \paragraph{Bjørn-Jostein Singstad}\label{bjuxf8rn-jostein-singstad}
...
Ridge regression and finally Lasso regression. The methods are in turn
combined with resampling techniques{[}1{]} \cite{granger2013}
...
\cite{hshsh}
    ...
    test citation \cite{hshsh}

    \subsection{Introduction}\label{introduction}

    Introduction: status of problem and the major objectives. Total number
of possible points: 10

    We will first study how to fit polynomials to a specific two-dimensional
function called Franke's function. This is a function which has been
widely used when testing various interpolation and fitting algorithms.
Furthermore, after having etsablished the model and the method, we will
employ resamling techniques such as the cross-validation and the
bootstrap methods, in order to perform a proper assessment of our models
{[}1{]}.

    In this case we will have two predictors and need to fit a function (for
example a polynomial) of x and y. Thereafter we will repeat much of the
same procedure using the the Ridge and Lasso regression methods,
introducing thus a dependence on the bias (penalty) λ. {[}1{]}

    \subsection{Formalism/methods}\label{formalismmethods}
...
\end{align}
    ...
    \$\$ MSE(y\^{},y\textsubscript{\^{})=1n∑i=0n−1(yi−y}i)2)

    \$
R2(y\textsuperscript{,y}\textsubscript{)=1−∑n−1i=0(yi−y}i)2∑n−1i=0(yi−y¯)2,

    y¯=1n∑i=0n−1yi

    

    Discussion of the methods used and their basis/suitability. Total number
of possible points 20

    \subsection{Code/Implementations/test}\label{codeimplementationstest}

    Code/Implementations/test: Readability of code, implementation, testing
and discussion of benchmarks. Total number of possible points 20
...
{\color{incolor}In [{\color{incolor}2}]:} \PY{k+kn}{from} \PY{n+nn}{mpl\PYZus{}toolkits}\PY{n+nn}{.}\PY{n+nn}{mplot3d} \PY{k}{import} \PY{n}{Axes3D}
        ...
        \PY{n}{y} \PY{o}{=} \PY{n}{np}\PY{o}{.}\PY{n}{arange}\PY{p}{(}\PY{l+m+mi}{0}\PY{p}{,} \PY{l+m+mi}{1}\PY{p}{,} \PY{l+m+mf}{0.05}\PY{p}{)}
        ...
        \PY{n}{z} \PY{o}{=} \PY{n}{FrankeFunction}\PY{p}{(}\PY{n}{x}\PY{p}{,} \PY{n}{y}\PY{p}{)}
...
{\color{incolor}In [{\color{incolor}6}]:} \PY{k+kn}{import} \PY{n+nn}{numpy} \PY{k}{as} \PY{n+nn}{np}
        ...
        \PY{n}{terrain1} \PY{o}{=} \PY{n}{imread}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{SRTM\PYZus{}data\PYZus{}Norway\PYZus{}1.tif}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}
...
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
        ...
        <ipython-input-6-3523ca5bce8f> in <module>()
          6 
          7 \# Load the terrain
    ----> 8 terrain1 = imread('SRTM\_data\_Norway\_1.tif')
          9 \# Show the terrain
         10 plt.figure()
    

        \textasciitilde{}\textbackslash{}Anaconda3\textbackslash{}lib\textbackslash{}site-packages\textbackslash{}imageio\textbackslash{}core\textbackslash{}functions.py in imread(uri, format, **kwargs)
        204 
        205     \# Get reader and read first
    --> 206     reader = read(uri, format, 'i', **kwargs)
        207     with reader:
        208         return reader.get\_data(0)
        ...
        115 
        116     \# Create request object
    --> 117     request = Request(uri, 'r' + mode, **kwargs)
        118 
        119     \# Get format
    

        \textasciitilde{}\textbackslash{}Anaconda3\textbackslash{}lib\textbackslash{}site-packages\textbackslash{}imageio\textbackslash{}core\textbackslash{}request.py in \_\_init\_\_(self, uri, mode, **kwargs)
        126 
        127         \# Parse what was given
    --> 128         self.\_parse\_uri(uri)
        129 
        130         \# Set extension
    

        \textasciitilde{}\textbackslash{}Anaconda3\textbackslash{}lib\textbackslash{}site-packages\textbackslash{}imageio\textbackslash{}core\textbackslash{}request.py in \_parse\_uri(self, uri)
        269                 \# Reading: check that the file exists (but is allowed a dir)
        270                 if not os.path.exists(fn):
    --> 271                     raise FileNotFoundError("No such file: '\%s'" \% fn)
        272             else:
        273                 \# Writing: check that the directory to write to does exist
    

        FileNotFoundError: No such file: 'C:\textbackslash{}Users\textbackslash{}bsingstad\textbackslash{}Documents\textbackslash{}GitHub\textbackslash{}FYS-STK4155-Project-1\textbackslash{}SRTM\_data\_Norway\_1.tif'

    \end{Verbatim}

    \subsection{Analysis/discussion}\label{analysisdiscussion}

    Analysis: of results and the effectiveness of their selection and
presentation. Are the results well understood and discussed? Total
number of possible points: 20

    \subsection{Conclusions}\label{conclusions}

    Conclusions, discussions and critical comments: on what was learned
about the method used and on the results obtained. Possible directions
and future improvements? Total number of possible points: 10

    \subsection{Clarity of figures and overall
presentation}\label{clarity-of-figures-and-overall-presentation}
...
Total number of possible points: 10*
    ...
    Referencing: relevant works cited accurately? Total number of possible
points 5*

{[}1{]} From the task
https://compphysics.github.io/MachineLearning/doc/Projects/2018/Project1/html/Project1-bs.html

    \subsection{Overall mark (\%) and final possible final
comments*}\label{overall-mark-and-final-possible-final-comments}
